...
class _ProjectOwnerProjectForm extends Component {
  ...
  handleSubmitEditedForm = async event => {
    ...
    const PROJECT_EDITED_SUCCESS_MESSAGE = !this._isProjectRejected(projectToRender.state) ? 
      `${projectToRender.title} details have been successfully updated!`: `${projectToRender.title} details have been successfully updated! It will now be pending admin approval.`;
    ...
    if (response.isSuccessful) {
      showAlert(
        ...
        AlertType.SUCCESS,
        PROJECT_EDITED_SUCCESS_MESSAGE
      );
      this.props.resetAllFields();
      this.resetAllSubFormFields();
      this.setState({ shouldRedirect: true });
